...
} from "./platform/backend.js";

import { registerHandlers } from "./handlers/index.js";
import { registerNativeHandlers } from "./platform/native-handlers.js";
import { registerHlsProtocol, transcodeService } from "./services/transcode.js";
  ...
  const menu = Menu.buildFromTemplate([
    {
      ...
      submenu: [
        ...
        { type: "separator" },
        {
          label: "Settings…",
          accelerator: "Command+,",
          click: () => ipcMain.broadcast("app:navigate", { to: "/settings" }),
        },
        { type: "separator" },
        { role: "services" },
        { type: "separator" },
...
app.whenReady().then(async () => {
  ...
  // Register the HLS scheme before any window is created (required timing).
  try {
    await registerHlsProtocol();
  } catch (error) {
    logger.error("main", "Failed to register HLS protocol", error as Error);
  }
